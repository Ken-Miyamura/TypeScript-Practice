/**
 * ジェネリクス型
 */

export default function genericsBasicSample() {
    // ジェネリクス型を使わない場合
    const stringReduce = (array: string[], initialValue: string): string => {
        let result = initialValue;
        for (let i = 0; i < array.length; i++) {
            result += array[i];
        }
        return result;
    }
    
    console.log(stringReduce(["May", "the", "force", "be", "with", "you"], ''));

    const numberReduce = (array: number[], initialValue: number): number => {
        let result = initialValue;
        for (let i = 0; i < array.length; i++) {
            result += array[i];
        }
        return result;
    }

    console.log(numberReduce([100, 200, 300, 400, 500, 600], 1000));

    // ジェネリック型を使う場合
    type genericReduce<T> = {
        (array: T[], initialValue: T): T
    }

    const genericStringReduce: genericReduce<string> = (array, initialValue) => {
        let result = initialValue;
        for (let i = 0; i < array.length; i++) {
            result += array[i];
        }
        return result;
    }

    console.log(stringReduce(["May", "the", "force", "be", "with", "us"], ''));

    const genericNumberReduce: genericReduce<number> = (array, initialValue) => {
        let result = initialValue;
        for (let i = 0; i < array.length; i++) {
            result += array[i];
        }
        return result;
    }

    console.log(numberReduce([100, 200, 300, 400, 500, 600], 2000));
}

